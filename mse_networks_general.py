...
while algebraic_connectivity < 1e-4:
    p = generate_topology(N, parameter)
    adjacency,laplacian,algebraic_connectivity,degree,coordinates = p
...
waux = np.zeros((M, 1)) # auxiliary Mx1 vector
baux = np.zeros((M, 1)) # auxiliary Mx1 vector
if same_model == 1: # all agents have the same model wo
    for k in range(1,N+1): # finding the limit point w_star in the MSE network case
        xd = mu[k-1]*p[k-1]*RU[:,k-1].reshape((M, 1)) 
        waux = waux + np.diag(xd.flatten()).dot(wo) 
        baux = baux + xd
else: # agents have different models w_{k}^o
    for k in range(1,N+1): # finding the Pareto solution w_star in the MSE network case
        xd = mu[k-1]*p[k-1]*RU[:,k-1].reshape((M, 1))
        waux = waux + np.diag(xd.flatten()).dot(Wo[:,k-1].reshape((M, 1)))
        baux = baux + xd
w_star = np.linalg.inv(np.diag(baux.flatten())).dot(waux) # limit point
#############################################
...
for L in range(Num_trial):
    ...
    for i in range(Num_iter):
        ...
        #CTA
        for k in range(N):
            psi_CTA[:, k] = np.zeros(M)
            for l in range(N):
                psi_CTA[:, k] = psi_CTA[:, k] + A[l, k]*w_CTA[:, l]
        
        #consensus
        for k in range(N):
            psi_CON[:, k] = np.zeros(M)
            for l in range(N):
...
MSD_av_db = 10 * np.log10(MSD_av)  # dB curve
...
MSD_av_db_CTA = 10 * np.log10(MSD_av_CTA)  # dB curve
...
MSD_av_db_CON = 10 * np.log10(MSD_av_CON)  # dB curve
...
if h == 1: # real data
    ...
    for k in range(N):
        ...
        zk = Wo[:,k] - w_star.flatten()    # perturbation between model w_k^o at agent k and w_star
        Wk = np.outer(zk, zk) 
        Rsk = 4*sigma_v2[k]*Ruk + 4*Ruk*np.trace(Wk.dot(Ruk)) + 4*Ruk.dot(Wk.dot(Ruk)) # R_{s,k}=4*sigma_{v,k}^2*R_{u,k}+4R_{u,k}Tr(W_k R_{u,k}+4 R_{u,k}W_{k}R_{u,k}
        Gk = Rsk
        S1 = S1 + mu[k]*p[k]*Hk
        S2 = S2 + mu[k]*mu[k]*p[k]*p[k]*Gk  
        S3 = S3 + mu[k]*np.linalg.inv(Hk).dot(Gk)  # for non-cooperarive solution (in this case, zk=0 and Gk reduces to Gk = 4*sigmav2(k)*Ruk)
        ...
        if k == 0:
            Hcal = sc.linalg.block_diag(Hk)
            Scal = sc.linalg.block_diag(Gk)
        else:
            Hcal = sc.linalg.block_diag(Hcal, Hk)
            Scal = sc.linalg.block_diag(Scal, Gk)
        if k == idx1: # for non-cooperative solution
            msdA =  (mu[k]/(2*h))*np.trace(np.linalg.inv(Hk).dot(Gk)) # MSD for non-cooperative agent of index idx1
            sigma_vA = sigma_v2[k] # corresponding noise variance.
            muA = mu[k] # step-size
            traceA = trace_values[k] # trace of covariance matrix
        if k == idx2:
            msdB =  (mu[k]/(2*h))*np.trace(np.linalg.inv(Hk).dot(Gk)) # MSD for non-cooperative agent of index idx1
...
#             traceB = trace_values[k] #trace of covariance matrix
#             muB = mu[k]      
MSD_thy_low_rank    = (1/(2*h))*np.trace(np.linalg.inv(S1).dot(S2))  # Theoretical MSD from the low-rank approximation formula
...
MSD_thy_small_mu_CEN    = (mu_max/(2*h*N))*np.trace(np.linalg.inv(S4).dot(S5))   # Theoretical MSD for small step-sizes for centralized solution
...
A1 = np.eye(N,N)
A2 = A
Mcal  = np.kron(np.diag(mu.flatten()),np.eye(h*M,h*M)) #15x15
Aocal = np.kron(Ao,np.eye(h*M,h*M)) #15x15
A1cal = np.kron(A1,np.eye(h*M,h*M)) #15x15
A2cal = np.kron(A2,np.eye(h*M,h*M)) #15x15
...
A1CTA = A
A2CTA = np.eye(N,N)
Mcal  = np.kron(np.diag(mu.flatten()),np.eye(h*M,h*M))
